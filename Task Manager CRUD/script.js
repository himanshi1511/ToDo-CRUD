...
document.getElementById("add-task-form").addEventListener("submit", function (event) {
        ...
        if (!isValidDueDate(dueDate)) {
            displayToast("Enter a valid date", "red");
            return;
        }
    ...
    // Function to check if the due date is valid (not earlier than today)
...
function displayToast(message, color) {
    ...
    // Hide the toast after a 2 seconds
    setTimeout(function () {
...
function startVoiceRecognition() {
    ...
    voiceStatusElement.textContent = "Listening..."; // Update status
    ...
    recognition.onend = function () {
        voiceStatusElement.textContent = "Speak now..."; // Reset status
    };
